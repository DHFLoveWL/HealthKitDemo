...
public extension Date {
    ...
    static func timestampToFormatterDate(timestamp: String) -> Date {
        ...
        guard let timestampInt = Date.toInt(timestamp) else {
            #if DEBUG
            fatalError("时间戳位有问题")
            #else
            return Date()
    ...
    private static func toInt(_ str: String) -> Int? {
...
public extension Date {
    ...
    /// 当前日期是不是润年
    var isLeapYear: Bool {
        let year = Date.currentDate.year
...
public extension Date {
    ...
    func componentCompare(from date: Date, unit: Set<Calendar.Component> = [.year,.month,.day]) -> DateComponents {
        let calendar = Calendar.current
        let component = calendar.dateComponents(unit, from: date, to: Date.currentDate)
        return component
    }
